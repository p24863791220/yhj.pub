...
Begin VB.Form IOCHECK5032 
   ...
   Begin VB.Label Label1 
      ...
      Caption         =   "AUDIOCHECK PIO Data Check"
      BeginProperty Font 
         Name            =   "宋体"
         Size            =   21.75
         Charset         =   134
...
Call IOwrite(5, 4) '气tui
    ...
    DoEvents
    Yy = GetTickCount() - Xx
    Loop Until IOin = 8 Or Yy > 1500 ' 检测tui到位
        If Yy > 1500 Then
        NGflag = 1
        Else
        Sleep (20)
...
Sleep (20)
    ...
    DoEvents
    Yy = GetTickCount() - Xx
    Loop Until IOin = 32 Or Yy > 1500 ' song kai
        If Yy > 1500 Then
        NGflag = 1
        Else
        Sleep (20)
...
If NGflag = 0 Then
    NGflag = 0
    Call IOwrite(5, 2) '气jin
    Sleep (20)
    Xx = GetTickCount()
    Do
    IOin = IOREAD
        DoEvents
    If (IOin And 2) = 2 Then
    ...
    IOin = IOin And 16
    Yy = GetTickCount() - Xx
    Loop Until IOin = 16 Or Yy > 1500 ' 检测顶杆推出到位
        If Yy > 1500 Then
        NGflag = 1
        Else
        Sleep (200)
...
If NGflag = 0 Then
    ...
    DoEvents
    Yy = GetTickCount() - Xx
    Loop Until IOin = 4 Or Yy > 2500 ' 检测顶杆推出到位
        If Yy > 2500 Then
        NGflag = 1
        Else
        Sleep (20)
...
Timer1.Enabled = False
   If (IOREAD And 8) = 8 And (IOREAD And 4) <> 4 Then
   SetIn = 0
   End If
   If (IOREAD And 8) <> 8 And (IOREAD And 4) = 4 Then
   SetIn = 1
