...
using MvCamera_SDK_CS_Demo;

namespace BasicDemo
{
    public partial class hkmvsbasic : Form
    {
        ...
        public hkmvsbasic()
        {
            ...
            hksavejpegpath = 螺丝自动锁付.AutoCheck.hksavejpegpath;
        }
        ...
        public void bnEnum_Click(object sender, EventArgs e)
        {
            DeviceListAcq();
        }
        ...
        public void bnContinuesMode_CheckedChanged(object sender, EventArgs e)
        {
        ...
        public void bnTriggerMode_CheckedChanged(object sender, EventArgs e)
        {
        ...
        public void bnStartGrab_Click(object sender, EventArgs e)
        {
        ...
        public void cbSoftTrigger_CheckedChanged(object sender, EventArgs e)
        {
            if (cbSoftTrigger.Checked)
            {
                // ch:触发源设为软触发 | en:Set trigger source as Software
        ...
        public void bnTriggerExec_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (MyCamera.MV_OK != nRet)
                {
                    ...
                    bnTriggerExec.BackColor = Color.Red;
                }
                bnTriggerExec.BackColor = Color.Green;
            }
            catch (Exception ex)
            {
                bnTriggerExec.BackColor = Color.Red;
        ...
        public void bnStopGrab_Click(object sender, EventArgs e)
        {
        ...
        public void bnSaveBmp_Click(object sender, EventArgs e)
        {
        ...
        public void bnSaveJpg_Click(object sender, EventArgs e)
        {
            ...
            lock (BufForDriverLock)
            {
                ...
                stSaveFileParam.nHeight = m_stFrameInfo.nHeight;
                stSaveFileParam.nWidth = m_stFrameInfo.nWidth;
                frameH.Text = stSaveFileParam.nHeight.ToString();
                frameW.Text = stSaveFileParam.nWidth.ToString();
                stSaveFileParam.nQuality = 80;
                stSaveFileParam.iMethodValue = 2;
                //stSaveFileParam.pImagePath = hksavejpegpath;
                stSaveFileParam.pImagePath =Application .StartupPath + @"\Rectangle\hkcatchpic\" + "Image_w" + stSaveFileParam.nWidth.ToString() + "_h" + stSaveFileParam.nHeight.ToString() + "_fn" + m_stFrameInfo.nFrameNum.ToString() + ".jpg";
                螺丝自动锁付.AutoCheck.hksavejpegpath = stSaveFileParam.pImagePath;
                int nRet = m_MyCamera.MV_CC_SaveImageToFile_NET(ref stSaveFileParam);
                //m_MyCamera.MV_CC_GetImageForRGB_NET();
                if (MyCamera.MV_OK != nRet)
                {
            ...
            //ShowErrorMsg("Save Succeed!", 0);
            bnSaveJpg.Enabled = true;
            bnSaveJpg.BackColor = Color.Green;
            watch.Stop();
            toolStripStatusLabel2.Text = watch.ElapsedMilliseconds.ToString();
        }

        public void bnSavePng_Click(object sender, EventArgs e)
        {
            if (false == m_bGrabbing)
            {
                ShowErrorMsg("Not Start Grabbing", 0);
        ...
        public void bnSaveTiff_Click(object sender, EventArgs e)
        {
            if (false == m_bGrabbing)
            {
                ShowErrorMsg("Not Start Grabbing", 0);
        ...
        public void bnGetParam_Click(object sender, EventArgs e)
        {
        ...
        public void bnSetParam_Click(object sender, EventArgs e)
        {
            try
            {
        ...
        public void loadset1_Click(object sender, EventArgs e)
        {
                int nRet = m_MyCamera. MV_CC_SetEnumValue_NET("UserSetSelector", 1);//切换不同的参数组
                if ( nRet!=0)
                {
                //Printf("error: SetEnumValue UserSetSelector fail [%x]\n", nRet);
                loadset1.BackColor = Color.Red;
                }
                nRet = m_MyCamera.MV_CC_SetCommandValue_NET("UserSetLoad");//load
                if (nRet != 0)
                {
                //printf("error: SetCommandValue fail [%x]\n", nRet);
                loadset1.BackColor = Color.Red;
                }
            loadset1.BackColor = Color.Green;
        }

        public void ipforce_Click(object sender, EventArgs e)
        {
            bnClose_Click(null,null);
            ForceIpDemo fcip = new ForceIpDemo();
            fcip.Show();
        }

        public void dis_reconnect_Click(object sender, EventArgs e)
        {
            bnClose_Click(null, null);
            ReconnectDemo.ReconnectDemo recd = new ReconnectDemo.ReconnectDemo();
            recd.Show();
        }

        public void loadset2_Click(object sender, EventArgs e)
        {
            int nRet = m_MyCamera.MV_CC_SetEnumValue_NET("UserSetSelector", 2);//切换不同的参数组
            if (nRet != 0)
            {
                //Printf("error: SetEnumValue UserSetSelector fail [%x]\n", nRet);
                loadset2.BackColor = Color.Red;
            }
            nRet = m_MyCamera.MV_CC_SetCommandValue_NET("UserSetLoad");//load
            if (nRet != 0)
            {
            ...
            loadset2.BackColor = Color.Green;
        }

        public void setdefault1_Click(object sender, EventArgs e)
        {
            //初始化部分，放在Opendevice之后即可
           int nRet = m_MyCamera.MV_CC_SetEnumValue_NET( "UserSetSelector", 1);
            if (nRet != 0)
            {
                //printf("error: SetEnumValue UserSetSelector fail [%x]\n", nRet);
                setdefault1.BackColor = Color.Red;
            }
            nRet = m_MyCamera.MV_CC_SetEnumValue_NET("UserSetDefault", 1);
            if (nRet != 0)
            {
                //printf("error: SetEnumValue UserSetDefault fail [%x]\n", nRet);
                setdefault1.BackColor = Color.Red;
            }
            setdefault1.BackColor = Color.Green;
        }

        public void saveset_Click(object sender, EventArgs e)
        {
            //设置Command型节点-发送参数保存命令
            //大部分参数修改完成后，调用此命令进行保存，不建议频繁调用
            int nRet = m_MyCamera.MV_CC_SetCommandValue_NET( "UserSetSave");
            if (nRet != 0)
            {
                //printf("error: SetCommandValue fail [%x]\n", nRet);
                saveset.BackColor = Color.Red;
            }
        ...
        private void hkmvsbasic_Load(object sender, EventArgs e)
        {
